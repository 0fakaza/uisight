...
function frameLoop(o) {
  ...
  // TWO gates, and they are not the same gate. `streaming` is the idle pause:
  // nobody is watching, so stop encoding. `gen` is session identity: this tick
  // belongs to a loop that has since been replaced or closed. A paused loop
  // resumes; a retired one must never come back.
  const live = () => o.streaming && o.page && o.streamGen === gen;
  const tick = async () => {
    if (!live()) return;
...
async function stopStream(o) {
  ...
  // clearTimeout, not clearInterval: the frame loop is setTimeout recursion now.
  // Node treats the two as interchangeable, so this was not a live bug -- but a
  // reader should not have to know that to believe the line.
  if (o.fallbackTimer) { clearTimeout(o.fallbackTimer); o.fallbackTimer = null; }
}
